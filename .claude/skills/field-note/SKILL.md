...
- Over-linking common knowledge (GitHub, CLI, API, "skill") — link products and companies people might want to explore, not things every engineer knows
- Underselling AI's contribution — when Xavier describes an AI-assisted workflow, represent the collaboration honestly. The AI often does real analysis, finds issues on its own, adds depth. Do not reduce it to "formatting and posting" or pure mechanics. Be precise: the human steers, the AI builds on that steering with its own intelligence.
- Overselling the human part — do not claim "the thinking is quick" or "the human does all the thinking" when AI contributes significant analysis. Be honest about the split.
- Overly specific technical examples that can be challenged — keep descriptions at a high level. If an example is so specific that someone could argue "that should not be done by a human anyway," it is too specific.
- Using "hot take" when the opinion is not actually controversial — prefer "my take" or "here is my take" for directional opinions many people might agree with.
- Dropping good short sentences between revisions — when Xavier approves a specific phrasing (e.g., "If the skill keeps making the same mistake, I improve the skill itself"), preserve it across edits. Track which lines Xavier explicitly liked.
